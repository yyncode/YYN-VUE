<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="navList" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" label="导航名称" prop="navName"/>
...
<script>
import {addNav, delNav, getNav, listNav, updateNav} from "@/api/news/nav";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除导航设置编号为"' + navIds + '"的数据项？').then(function () {
        return delNav(navIds);
      }).then(() => {
        this.getList();
